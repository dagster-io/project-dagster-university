...
def test_dbt_assets(setup_dbt_env):  # noqa: F811
    from dagster_and_dbt.completed.lesson_4.defs.assets import dbt
    from dagster_and_dbt.completed.lesson_4.defs.resources import dbt_resource
...
def test_defs(setup_dbt_env):  # noqa: F811
    assert dg.Definitions.merge(
        dg.components.load_defs(dagster_and_dbt.completed.lesson_4.defs)
    )
