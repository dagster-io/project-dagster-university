...
This upfront review is valuable precisely because it happens before anything is written. If the plan surfaces a decision you hadn't considered—do we want daily or hourly partitions? should the new resource go in `resources.py` or a new file?—you resolve it at planning time, not mid-implementation when reverting is messier.
