...
You'll see this pattern directly in Lesson 4. The skill-less agent has to reason over open-ended questions — which layout, which package manager, whether to validate — drawing on broad context. The skill-driven agent calls `dg scaffold` and `dg check defs`, then stops. Each command returns a small, structured response. That's the token difference in practice: not a theoretical claim but a visible pattern in the agent's behavior.
...
![Prompt with the dg](/images/ai-driven-data-engineering/lesson-4/prompt-agent-dg.png)
