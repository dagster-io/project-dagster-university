...
In this lesson, you'll learn how to use **Sling** with Dagster for database replication. We'll cover:

- **Sling fundamentals**: Declarative YAML-based configuration for replication
- **Replication modes**: When to use `full-refresh`, `incremental`, `append`, and `truncate`
- **Incremental loading**: Using `update_key` columns to load only new or changed data
- **State management**: How Sling tracks progress using state files via `SLING_STATE`
- **Dagster integration**: Using `@sling_assets` and `SlingResource`
- **Schema evolution**: How Sling handles schema changes automatically
